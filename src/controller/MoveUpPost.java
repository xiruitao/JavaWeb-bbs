...
public class MoveUpPost extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        String id1 = request.getParameter("post_id");//发出上移请求的帖子(属于置顶的帖子)的ID
        int id = 0;
        if (id1 == null || id1.equals("")) {
            ...
            try{
                ...
                if(postValue == posts.size()){
                    response.getWriter().println(ResJson.generateResJson(0, "Already the top",id));
                }else{
                    ...
                    if(rs==1&&rs3==1)
                        response.getWriter().println(ResJson.generateResJson(1, "Move Up Successful", id));
                    else
                        response.getWriter().println(ResJson.generateResJson(0, "Move Up Unsuccessful","none" ));
                }
            }catch (Exception e){
                e.printStackTrace();
            }
